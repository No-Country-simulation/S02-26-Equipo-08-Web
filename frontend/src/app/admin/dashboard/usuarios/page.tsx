...
import { listarUsuarios, cambiarEstadoUsuarioAction } from "@/src/actions/usuarios";
...
export default function UsuariosPage() {
    ...
    const [accionPendiente, setAccionPendiente] = useState<{ id: number; nuevoEstado: number } | null>(null);
    ...
    // abrir modal con los datos de la accion a confirmar
    const abrirModal = (usuario: Usuario, nuevoEstado: number, accion: string) => {
        ...
        setAccionPendiente({ id: usuario.id, nuevoEstado });
        setModalOpen(true);
    };
                                            ...
                                            >
                                                ...
                                                <td className="px-6 py-4 text-slate-500 text-sm">
                                                    <div className="flex gap-2">
                                                        ...
                                                        {(usuario.estado === "Pendiente de Aceptar" || usuario.estado === "Rechazado" || usuario.estado === "Desactivado") && (
                                                            <button
                                                                title="Aceptar / Activar"
                                                                onClick={() => abrirModal(usuario, 2, usuario.estado === "Pendiente de Aceptar" ? "Aceptar" : "Activar")}
                                                                className="p-1 hover:bg-emerald-50 rounded-md text-emerald-600 transition-colors cursor-pointer"
                                                            >
                                                                <UserCheck size={18} />
                                                            </button>
                                                        )}
                                                        {usuario.estado === "Pendiente de Aceptar" && (
                                                            <button
                                                                title="Rechazar"
                                                                onClick={() => abrirModal(usuario, 3, "Rechazar")}
                                                        ...
                                                        {usuario.estado === "Activo" && (
                                                            <button
                                                                title="Desactivar"
                                                                onClick={() => abrirModal(usuario, 4, "Desactivar")}
                                                                className="p-1 hover:bg-red-50 rounded-md text-red-500 transition-colors cursor-pointer"
                                                            >
                                                                <UserX size={18} />
                                                            </button>
